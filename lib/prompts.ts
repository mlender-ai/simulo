...
/** Detects whether the targetUser description references 4050 Korean women. */
function targetIncludes4050(targetUser?: string): boolean {
  if (!targetUser) return false;
  const t = targetUser.toLowerCase();
  return (
    /4050|40-50|40~50|40·50|40대|50대|중년|주부/.test(targetUser) ||
    /(^|\D)40s?\b|(^|\D)50s?\b|middle[- ]?aged/.test(t)
  );
}
...
/**
 * Desire alignment layer. Only injects YafitMove-specific 4050 desire map
...
function desireLayer(locale: string, targetUser?: string): string {
  ...
  if (is4050) {
    return isKo
      ? `## 옵션: 욕망 충족도 분석 — 4050 한국 여성 타깃

이 분석은 4050 한국 여성(주로 야핏무브류 리워드 앱 주 사용자)을 타깃으로 합니다.
...
추가 JSON 필드:
{
  "desireAlignment": {
    ...
    "lossAversion": { "score": 0-10, "comment": "[Evidence] ... → [Judgment] 손실 회피 활성화 여부" }
      ...
      : `## Option: Desire Alignment — 4050 Korean women target

This analysis targets Korean women aged 40-50s (core users of YafitMove-style reward apps).
...
const ACCESSIBILITY_4050_LAYER_EN = `## Option: 4050 Accessibility Evaluation

Evaluate two accessibility dimensions that matter most for users aged 40-50s.
...
const ACCESSIBILITY_4050_LAYER_KO = `## 옵션: 4050 접근성 평가

4050 유저에게 가장 중요한 두 가지 접근성 차원을 평가합니다.
...
export const SYSTEM_PROMPT_KO = buildSystemPrompt({
  mode: "hypothesis",
  analysisOptions: { desireAlignment: true, accessibility: true },
  targetUser: "4050 한국 여성, 야핏무브 핵심 타깃",
  inputShape: "single",
  locale: "ko",
});
